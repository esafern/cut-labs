...
> **SIP Note:** On modern macOS (Catalina+), System Integrity Protection restricts dtrace. To use kernel probes, you need to boot into Recovery Mode and run `csrutil enable --without dtrace`. For architecture study, the userspace probes below work without SIP changes.
...
# OR: Java client with mTLS (more architecture-relevant)
```
